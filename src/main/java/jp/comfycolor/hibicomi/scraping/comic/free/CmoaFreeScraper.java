...
import jp.comfycolor.hibicomi.scraping.comic.detail.CmoaDetailScraper;
import jp.comfycolor.hibicomi.utils.scraping.ScrapeUtils;

public class CmoaFreeScraper extends BaseFreeScraper {

	public CmoaFreeScraper(SettingBean setting, SiteBean site) {
		super(setting, site);

		detailScraper = new CmoaDetailScraper(driver, js, setting, site);
	}
	...
	public boolean scrapeUrl(PageBean page, String url) throws Exception {
		...
		for ( WebElement dive : driver.findElements(By.className("sub_contents_box")) ) {
			BookInfoBean bookInfo = new BookInfoBean(setting);

			// 詳細URL取得
			WebElement detailE = dive.findElement(By.xpath("div/p[@class='titile_name']/a"));

			logger.debug("■■ "
					+ ", タイトル: "+ detailE.getText()
					+ ", URL: "+ detailE.getAttribute("href")
					);

			bookInfo.setBook_site_url(detailE.getAttribute("href"));
		...
		// BL・TL
		for ( WebElement dive : driver.findElements(By.className("radius-table")) ) {
			BookInfoBean bookInfo = new BookInfoBean(setting);

			// 詳細URL取得
			WebElement detailE = dive.findElement(By.xpath("div/div/p[@class='titile_name']/a"));
		...
		if (!isExistedStop && !isLimitOver(page)) {
			// 次へのリンクが有効な状態である場合
			List<WebElement> nextEs = driver.findElements(By.xpath("//p[@class = 'p_active']/following-sibling::li"));
			if (nextEs.size() > 0) {
				// 「次へ」押下
				logger.debug("<<NEXT>> -----------------------------------------");
